...
async function main() {
  console.log("🌱 Seeding database (safe upsert — existing bookings preserved)...");
  ...
  const roomData = [
    {
      ...
      tags: JSON.stringify([
        "28 m²", "1 Extra-Long King Bed", "Private Bathroom & Bidet",
        "Lake View", "Garden View", "Pool View", "Balcony & Terrace",
        "Barbecue", "Air Conditioning", "Free Wi-Fi", "Smoking Permitted",
      ]),
      gallery: JSON.stringify([
        "/images/rooms/deluxe-main.jpg", "/images/rooms/double-bed.jpg",
        "/images/rooms/room-interior-1.jpg", "/images/rooms/room-interior-2.jpg",
        "/images/rooms/bed-detail.jpg", "/images/rooms/bathroom.jpg", "/images/rooms/vanity.jpg",
      ]),
    ...
    },
    {
      ...
      tags: JSON.stringify([
        "85 m²", "1 King Bed + 1 Sofa Bed", "Private Kitchen", "Washing Machine",
        "Lake View", "Garden View", "Pool View", "Balcony & Terrace",
        "Barbecue", "Air Conditioning", "Free Wi-Fi", "Private Bathroom & Bidet", "Smoking Permitted",
      ]),
      gallery: JSON.stringify([
        "/images/rooms/apt-living.jpg", "/images/rooms/apt-interior-1.jpg",
        "/images/rooms/apt-kitchen.jpg", "/images/rooms/bathroom-shower.jpg",
    ...
    },
    {
      ...
      tags: JSON.stringify([
        "4 Double Rooms", "Lake Apartment", "Lush Garden", "Pool Access",
        "Lake Activities", "Lake View", "Garden View", "Free Wi-Fi",
        "Total Privacy", "Up to 12 Guests",
      ]),
      gallery: JSON.stringify([
        "/images/villa/master-suite.jpg", "/images/hero/villa-exterior.jpg",
        "/images/gallery/villa-architecture.jpg", "/images/gallery/pool-reflection.jpg",
      ]),
  ...
  console.log(`✅ Upserted ${roomData.length} rooms`);

  // ─── Upsert Offers / Promo Codes ──────────────────────────────────────────
  const offerData = [
    {
      code: "HSVHONEY",
  ...
  console.log(`✅ Upserted ${offerData.length} offers/promo codes`);
  ...
  for (const item of diningData) {
    const existing = await prisma.diningItem.findFirst({ where: { title: item.title } });
    if (!existing) {
      await prisma.diningItem.create({ data: item });
    }
  }
  console.log(`✅ Upserted ${diningData.length} dining items`);
  ...
  console.log(`✅ Upserted admin user: adminhikka`);

  // ─── NOTE: Bookings are NEVER deleted here ─────────────────────────────────
  // Real guest bookings made through the site are preserved across deploys.
  // Only create 3 demo bookings if the bookings table is completely empty.
      ...
      data: [
        {
          ...
          adults: 2, children: 0, guestType: "foreign",
          notes: "Honeymoon trip — please prepare flower arrangements.",
          promoCode: "HSVHONEY", discountPercent: 10,
          basePrice: 85, nights: 3, totalPrice: 229.5, advancePayment: 114.75,
          status: "confirmed",
        },
        {
          ...
          adults: 2, children: 2, guestType: "local",
          notes: "Family holiday. Children are 5 and 8 years old.",
          promoCode: "", discountPercent: 0,
          basePrice: 150, nights: 5, totalPrice: 750, advancePayment: 375,
          status: "pending",
        },
        {
          ...
          adults: 8, children: 3, guestType: "foreign",
          notes: "Corporate team retreat. Please arrange for BBQ on arrival night.",
          promoCode: "HSVVILLA", discountPercent: 5,
          basePrice: 490, nights: 6, totalPrice: 2793, advancePayment: 1396.5,
          status: "confirmed",
        },
      ],
    });
    console.log(`✅ Created 3 demo bookings (table was empty)`);
  } else {
    console.log(`ℹ️  Skipped demo bookings — ${bookingCount} real booking(s) already exist`);
  }
